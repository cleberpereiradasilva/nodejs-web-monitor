...
//localhost:3000/status/?url=https://www.cvc.com.br/promoca/top-destinos/&slackKey=/T053BAYF0/BPBUACWF7/ib5mB54sGnO2kfjaT9niMg5G&statusCode=200
app.get("/status", async function(req, res) {
  ...
  getStatusHttps(url, function(props) {
    if (props.data && props.statusCode !== statusCode) {
      sendSlack(
        `${URL_SLACK}${slackKey}`,
        `Código "${props.statusCode}" não esperado. Era esperado ${statusCode}`
      );
    }
  });
  res.send({ message: "All Okey" });
